...
import com.sebas.prueba.tecnica.task.basic_tasks_manager.application.task.service.impl.TaskCreateApplicationServiceImpl;
...
class TaskCreateApplicationServiceImplTest {

    @InjectMocks
    private TaskCreateApplicationServiceImpl taskCreateApplicationService;
    ...
    void createTask() throws TaskSQLCustomException {
        ...
        when(taskCreateService.createTask(any()))
                .thenReturn(expectedTask);
        when(taskDTOMapper.fromTaskToTaskDTO(any()))
                .thenReturn(expectedTaskDTO);

        TaskDTO taskDTO = taskCreateApplicationService.createTask(expectedTaskDTO);

        assertCreateTaskDTO(expectedTaskDTO, taskDTO);
    }

    private static void assertCreateTaskDTO(TaskDTO expectedTaskDTO, TaskDTO taskDTO) {
